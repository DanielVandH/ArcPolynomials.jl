...
@testset "TrigonometricSpline" begin
    ...
    for points in (p1, p2, p3, p4)
        ...
        S = AP.TrigonometricSpline(points)
        @test axes(S) == (Inclusion(AP.ArcInclusion(-1)), Base.OneTo(length(points) - 1))
    ...
    @testset "ForwardDiff" begin
        ...
        spl = AP.TrigonometricSpline([-pi / 2, 0.1, 0.3, 1.0, 2.5, 2.6, 3.0, 4.5, 3pi / 2] .- pi/2)
